...
class Escrito_pergunta extends Escrito_pergunta_m {
  ...
  function tabelaEscrito_pergunta_html($where = "", $caminho = "", $atualizar = "", $ondeAtualizar = "", $apenasLinha = false) {
    ...
    if ($array) {
      ...
      foreach ($array as $iten) {
        ...
        if( $Pergunta->get_pergunta_idPergunta() ) continue;
        
        $Tipopergunta = new Tipopergunta($Pergunta -> get_tipoPergunta_idPergunta());
        
        $titulo = $Pergunta -> get_tituloPergunta();
        if( $Pergunta->get_tipoPergunta_idPergunta() == "5" ){
          $titulo .= " <span style=\"color:red\">[possui questões vinculadas]<span>";
        }
  ...
  function cadastrarEscrito_pergunta($idEscrito_pergunta, $post = array()) {
    ...
    if ($escrito_id == '')
      return array(false, MSG_OBRIGAT . " Escrito");
    ...
    if ($idEscrito_pergunta)
      $where .= " AND id NOT IN (" . Uteis::escapeRequest($idEscrito_pergunta) . ") ";
    $rs = $this -> selectEscrito_pergunta($where, array("id"));
    if ($rs)
      return array(false, "A pergunta já está vinculada a este teste escrito");

    //SETAR
    $this -> set_escrito_idEscrito_pergunta($escrito_id) -> set_pergunta_idEscrito_pergunta($pergunta_id);
    ...
    if ($idEscrito_pergunta) {
      ...
      $resultado = $this -> insertEscrito_pergunta();
      
      //VINCULA PERGUNTAS DEPENDENTES AO TESTE
      $Escrito_pergunta = new Escrito_pergunta();
      $Pergunta = new Pergunta($pergunta_id);

      if ($Pergunta -> get_tipoPergunta_idPergunta() == "5") {
        $rs = $Pergunta -> selectPergunta(" WHERE excluido = 0 AND pergunta_id = " . $Pergunta -> get_idPergunta(), array("id"));
        foreach ($rs as $value) {
          $post["pergunta_id"] = $value['id'];
          $Escrito_pergunta -> cadastrarEscrito_pergunta("", $post);
        }
      }
