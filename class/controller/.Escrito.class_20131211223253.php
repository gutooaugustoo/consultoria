	...
	function tabelaEscrito_html($where = "", $caminho = "", $atualizar = "", $ondeAtualizar = "", $apenasLinha = false){
		...
		if( $array ){
			...
			foreach($array as $iten){
				...
				$colunas[] = $Etapa -> get_idEtapa();
				$Tipoescrito = new Tipoescrito( $this -> get_tipoEscrito_idEscrito() );
				$colunas[] = $Tipoescrito -> get_idTipoescrito();
				$Servico = new Servico( $this -> get_servico_idEscrito() );
				$colunas[] = $Servico -> get_idServico();
				$colunas[] = $this -> get_randomicoEscrito(true);
